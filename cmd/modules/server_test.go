...
func TestServer_RejectsTmpBakAndHiddenFiles(t *testing.T) {
	dir := t.TempDir()
	logger, err := NewLogger(t.TempDir())
	if err != nil {
		t.Fatalf("NewLogger: %v", err)
	...
	// Create sensitive files in public directory.
	if err := os.WriteFile(dir+"/metrics.json.tmp", []byte(`{"staging": true}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(dir+"/metrics.json.bak", []byte(`{"backup": true}`), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(dir+"/.env", []byte(`SECRET=123`), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	for _, p := range sensitivePaths {
		t.Run(p, func(t *testing.T) {
			...
			if resp.StatusCode != http.StatusNotFound {
				t.Errorf("path %s: want 404 Not Found, got %d", p, resp.StatusCode)
			}
		})
	}
}
