...
// stages progress to a temporary metrics file, and commits the completed dataset to FileManager.
func RunPing(ctx context.Context, cfg *Config, fm *FileManager, logger *Logger, ts time.Time) {
	...
	if err := fm.StartTask(); err != nil {
		logger.Error("failed to start ping task", "error", err)
		return
	}
	defer fm.AbortTask()
	...
	if err := fm.CommitTask(); err != nil {
		logger.Error("failed to commit ping metrics", "error", err)
	}
