...
const ui = {
    ...
    latencyTimeFilter: getElement("latencyTimeFilter"),
    latencyTimeControlGroup: getElement("latencyTimeControlGroup"),
...
function renderLatencyChart(data, protocolFilter, targetFilter, timeFilter, text, grid, palette) {
    ...
    const targetEntries = toLatencySeriesEntries(buildLatencySeries(filteredData, protocolFilter, targetFilter));
...
function applyTheme(theme) {
    ...
    if (rawData.length > 0) {
        const data = getFilteredData();
        if (data.length > 0) {
            renderCharts(data, ui.protocolFilter.value, ui.latencyTargetFilter.value, getLatencyTimeFilter());
        }
    }
}
