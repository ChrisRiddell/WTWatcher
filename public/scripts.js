...
function parseData(json) {
    const dates = new Set();
    rawData = Object.entries(json).flatMap(([date, times]) => Object.entries(times).map(([time, entry]) => {
...
function getFilteredData() {
    ...
    return rawData.filter(d => d.date === date &&
        (!d.latency ||
            protocol === 'Both' ||
    ...
    data.forEach((d, i) => {
        Object.entries(d.latency ?? {}).forEach(([target, entries]) => {
            entries.forEach(e => {
                ...
                if (!targetMap[key])
                    targetMap[key] = Array(data.length).fill(null);
