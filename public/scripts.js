...
const DEFAULT_TIME_FILTER = "today";
...
function isProtocolFilter(value) {
    return isProtocol(value) || value === "IPv4 + IPv6";
}
function isLatencyTimeFilter(value) {
    return (value === "today" || value === "1h" || value === "5h" || value === "12h");
}
...
// DOM
// -----------------------------------------------------------------------------
function getElement(id, isType) {
    const element = document.getElementById(id);
    if (!element) {
        throw new Error(`Required element "${id}" was not found.`);
    }
    if (!isType(element)) {
        throw new Error(`Required element "${id}" has an unexpected HTML element type.`);
    }
    return element;
}
const ui = {
    dateFilter: getElement("dateFilter", (element) => element instanceof HTMLSelectElement),
    protocolFilter: getElement("protocolFilter", (element) => element instanceof HTMLSelectElement),
    latencyTargetFilter: getElement("latencyTargetFilter", (element) => element instanceof HTMLSelectElement),
    latencyTimeFilter: getElement("latencyTimeFilter", (element) => element instanceof HTMLSelectElement),
    latencyTimeControlGroup: getElement("latencyTimeControlGroup", (element) => element instanceof HTMLElement),
    themeToggle: getElement("themeToggle", (element) => element instanceof HTMLButtonElement),
    status: getElement("statusContainer", (element) => element instanceof HTMLElement),
    error: getElement("errorAlert", (element) => element instanceof HTMLElement),
    empty: getElement("emptyAlert", (element) => element instanceof HTMLElement),
    loading: getElement("loadingSkeleton", (element) => element instanceof HTMLElement),
    main: getElement("mainContent", (element) => element instanceof HTMLElement),
    speedCard: getElement("speedtestCard", (element) => element instanceof HTMLElement),
    speedSection: getElement("speedtestChartSection", (element) => element instanceof HTMLElement),
    latencyCards: getElement("latencyCardsContainer", (element) => element instanceof HTMLElement),
    latestDownload: getElement("latestDownload", (element) => element instanceof HTMLElement),
    latestUpload: getElement("latestUpload", (element) => element instanceof HTMLElement),
    speedTime: getElement("speedtestTime", (element) => element instanceof HTMLElement),
};
// -----------------------------------------------------------------------------
// CSS / Theme Helpers
// -----------------------------------------------------------------------------
...
function normalizeLatency(value) {
    ...
    for (const [target, rawEntries] of Object.entries(value)) {
        ...
        const entries = rawEntries.flatMap((rawEntry) => {
            ...
            const average = toNonNegativeFiniteNumber(rawEntry.Average ?? rawEntry.average, DEFAULT_LATENCY);
            const packetLoss = Math.min(100, toNonNegativeFiniteNumber(rawEntry.PacketLoss ?? rawEntry.packetLoss, DEFAULT_PACKET_LOSS));
            return [
                {
                    average,
...
function getLatencyTimeCutoff(data, selectedDate, timeFilter) {
...
function filterLatencyDataByTime(data, selectedDate, timeFilter) {
...
function buildLatencyHistory(data) {
    ...
    for (const point of data) {
        ...
        for (const [target, entries] of Object.entries(point.latency)) {
            const targetHistory = history.get(target) ?? new Map();
...
function getLatencyStatus(baseline, latest) {
    if (latest === 0) {
        return {
            cls: "status-ghost",
            label: "No Data",
...
function getWorstStatus(entries) {
    if (entries.length === 0) {
        return {
            cls: "status-ghost",
            label: "No Data",
...
function updateLatencyCards(data, protocolFilter) {
    ...
    if (stats.length === 0 && ui.speedCard.classList.contains("hidden")) {
        ui.latencyCards.appendChild(createElement("div", {
...
function findLatest(data, selector) {
    for (let index = data.length - 1; index >= 0; index -= 1) {
...
function toLatencySeriesEntries(series) {
    return [...series.entries()].map(([key, data]) => {
        const validValues = data.latency.filter((value) => value !== null);
        return {
            key,
            ...data,
...
function createLatencyDatasets(entries, palette) {
...
function renderLatencyChart(data, protocolFilter, targetFilter, timeFilter, text, grid, palette) {
    charts.latency?.destroy();
    const filteredData = filterLatencyDataByTime(data, ui.dateFilter.value, timeFilter);
    const latencyData = filteredData.filter((point) => point.latency);
    const labels = latencyData.map((point) => point.formattedTime);
    const seriesEntries = toLatencySeriesEntries(buildLatencySeries(filteredData, protocolFilter, targetFilter));
    const latencyCtx = getElement("latencyChart", (element) => element instanceof HTMLCanvasElement);
...
function renderSpeedtestChart(data, grid) {
    ...
    const speedCtx = getElement("speedtestChart", (element) => element instanceof HTMLCanvasElement);
...
function renderCharts(data, protocolFilter, targetFilter, timeFilter) {
    destroyCharts();
    ...
    renderLatencyChart(data, protocolFilter, targetFilter, timeFilter, text, grid, palette);
...
function updateLatencyChart() {
    ...
    const { text, grid } = getThemeColors();
    renderLatencyChart(data, getSelectedProtocol(), ui.latencyTargetFilter.value, getLatencyTimeFilter(), text, grid, getChartPalette());
}
// -----------------------------------------------------------------------------
// Filter Population
// -----------------------------------------------------------------------------
...
function applyFilters() {
    ...
    renderCharts(data, protocolFilter, ui.latencyTargetFilter.value, getLatencyTimeFilter());
}
...
function applyTheme(theme) {
    ...
    renderCharts(data, getSelectedProtocol(), ui.latencyTargetFilter.value, getLatencyTimeFilter());
}
